...
double timer::split()
{
    ...
    return chrono::duration_cast<chrono::duration<double>>(now - start_t).count();
}
...
double timer::total_duration_in_sec()
{
    if(not stopped)
    {
        throw runtime_error("Can't calculate total diration before stopping the timer");
    }
    return chrono::duration_cast<chrono::duration<double>>(stop_t - start_t).count();
}
...
vector<double> timer::durations_in_sec()
{
    ...
    {
        all_durations.push_back(chrono::duration_cast<chrono::duration<double>>(full_record[i]-full_record[i-1]).count());
    }
...
vector<long long> timer::durations_in_ms()
{
    ...
    for(unsigned int i = 1; i < full_record.size(); ++i)
    {
        all_durations.push_back(chrono::duration_cast<chrono::milliseconds>(full_record[i]-full_record[i-1]).count());
    }
    return all_durations;
}
...
vector<chrono::duration<double>> timer::durations()
{
    ...
    for(unsigned int i = 1; i < full_record.size(); ++i)
    {
        all_durations.push_back(chrono::duration_cast<chrono::milliseconds>(full_record[i]-full_record[i-1]));
    }
    return all_durations;
}
