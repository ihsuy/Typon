...
{   // needed later for controlling menu behaviour
    ...
    int wpm_w = id_w + 4;
    int mis_w = id_w + 3;
    inHistory_header_spacing = {
        id_w, wpm_w, mis_w, fixed_date_w
    };
...
{   // 1. check if record exists for this quote ID
    ...
    {   // check if record exists
        ...
        {   // check if input size matches the time record size
            if (best_record.second.first.size() != best_record.second.second.size())
            {
                remove_best_record(1, "Record for this quote(ID: "
                                   + to_string(quote_order[quoteOrder_index])
                                   + ") is modified (psave time record and input size don't match)\n"
                                   + "Do you want to remove this record?");
                mode = "Classic"; // do not start challenge mode
            }
            else
            {   // is this the correct record for this quote?
                ...
                {   // Ok, safe to activated Challenge mode
                    G.info_add_str("Challange Mode Activated", challenge_warning_color, true, false);
                    G.info_add_str("Best Gross WPM: "
                                   + d_to_string(best_record.first, 2),
    ...
    if (mode == "Classic")
    {
        ...
        if (intro_info_wait)
        {   // don't clear previous since there's extra message for no record
            // very inefficient work around with a boolean, improvement needed
            clear_prev = false;
...
{
    while (not current_game.game_complete()
            and not skip_game)
    {
        inGame_controller();

        // features disabled because
        // they somehow slow the program down
...
void game_engine::Challenge_inGame_loop()
{
    ...
    // variable "current_game_terminated" is only used to
    // interrupt oppoenent thread in challenge mode
...
void game_engine::opponents_play()
{
    ...
    for (auto i = 0; i < best_inp.size(); ++i)
    {
        if (current_game_terminated)
        {
            //early stop
            timing_end();
            // substract last waiting time from time_elapsed since it was early stopped
...
void game_engine::game_end()
{
    ...
    if (not skip_game)
    {   // important to only calculate and save performance
        // if game is properly completed
        current_game.calculate_performance();

        if (isDemo)
        {   // do not save in file for demo mode
            // hole them in memory for 1 time use
            if (current_game.gross_wpm > demo_best_record.first)
            {
                auto demo_time_rec = inner_stopwatch.durations_in_ms();
                demo_time_rec.pop_back();
                demo_best_record = make_pair(current_game.gross_wpm, make_pair(demo_time_rec, all_inputs));
            }
        }
        else
        {   // if not in demo mode
            // save to save files
            saver.save(quote_order[quoteOrder_index],
                       current_game.gross_wpm,
                       current_game.mistake_count,
                       inner_stopwatch.durations_in_ms(),
                       all_inputs);
        }
    }
}
...
void game_engine::finalize()
{
    ...
    // inGameWPM() is too slow
    // inGame_WPM = vector<double>();
...
void game_engine::inGame_get_input()
{   // ignore all characters except ascii
...
void game_engine::inGame_controller()
{   // control in-game key response
    inGame_get_input();
...
void game_engine::progress()
{   // game advances 1 cycle
...
void game_engine::replay()
{
    ...
    for (auto i = 0; i < all_inputs.size(); ++i)
    {
        ...
        {   // rapidly run to the end
            this_thread::sleep_for(chrono::duration<long double>(replay_skipToEnd_speed));
        }
        ...
        G.info_add_str("Now replaying...\nWPM: "
                       + d_to_string(overall_gross_wpm, 2),
                       now_replaying_color, true, false);

        show_bottom_msg(inReplay_bottom_msg);

        //G.info_add_str("current gross WPM: " + to_string(mock_wpm), "yellow", false, false, "\n");

        G.draw_keyboard(0, G.info_w / 2 - 10, all_inputs[i], replay_keyboard_color);
    }
    ...
    erase_bottom_msg();
    G.info_add_str("overall gross WPM: "
                   + d_to_string(overall_gross_wpm, 2),
                   now_replaying_color, true, false);
    G.info_mvadd_str(G.inner_info_h - 1, 0, "replay is over. Press SPACE to continue",
...
void game_engine::inHistory_controller(long long& start_i, long long& end_i)
{
    ...
    case ARROW_RIGHT: case KEY_d:
    {
        verticle_move = false;
        inHistory_headerChoiceMove(RIGHT);
        break;
    }
    case ARROW_LEFT: case KEY_a:
    {
        verticle_move = false;
...
void game_engine::open_menu(const string& menu_type, void (game_engine::*controller)())
{
    inMenu = true;
    menuChoice = 0;
...
{
    if (not isDemo) saver.update_nchar(all_inputs);
...
void game_engine::print_header(const int& highlight)
{
    ...
    for (int i = 0; i < header_choices.size(); ++i)
    {
        if (i == highlight)
        {
            G.text_mvadd_str(0, inHistory_header_position[i], header_choices[i], inHistory_header_highlight_color);
        }
        else
        {
            G.text_mvadd_str(0, inHistory_header_position[i], header_choices[i], inHistory_header_color);
        }
    }
...
void game_engine::print_record(const int& highlight, vector<string>& history_rec)
{
    if (history_rec.size() == 0)
    {   // if no record in gsave
    ...
    else
    {
        ...
        if (not isDemo and verticle_move)
        {
            ...
            temp_quote = q_loader.load_quote(highlighted_quote_id, inHistory_quoteGlancee_len);

            if (temp_quote == "")
            {
                G.erase_info();
                G.info_mvadd_str(0, 0, "Quote not found.", inHistory_quoteGlance_color, false, false);
            }
            else
            {
                G.erase_info();
                G.info_mvadd_str(0, 0, temp_quote + "...", inHistory_quoteGlance_color, false, false);
            }
        }
    }
}

void game_engine::inHistory_recordChoiceMove(const int& val, long long& start_i, long long& end_i)
{
    verticle_move = true;
...
void game_engine::show_History()
{
    ...
    gsave_page_maxnum = ((saver.gsave_len <= max_hdisplay_nline)
                         ? 1
    ...
    while (inHistory)
    {
        ...
        // show page number
        string page_info = "Page:" + to_string(gsave_page_num) + "/" + to_string(gsave_page_maxnum);
        G.info_mvadd_str(0,
                         G.inner_info_w - (int)page_info.length(),
                         page_info,
...
void game_engine::inHistory_skipToQuote()
{
    ...
    if (current_info_input == KEY_y)
    {
        ...
        if (temp_quote == "")
        {
            G.info_mvadd_str(2, 0, "Quote not found. Press anything to continue.", "black-yellow");
            inInfo_get_input();
        }
        else
        {
...
{
    if (item != "id") historyHeader_isAscend["id"] = true;
    if (item != "wpm") historyHeader_isAscend["wpm"] = false;
    if (item != "mistake") historyHeader_isAscend["mistake"] = true;
    if (item != "date") historyHeader_isAscend["date"] = true;
}
...
void game_engine::show_stats()
{
    ...
    {
        G.text_mvadd_str(i * spacing_multiplier, 2, stats[i], "yellow", false, false);
    }
...
void game_engine::skipToQuote()
{
    ...
    while (true)
    {
        if (input_error)
        {
            G.info_add_str("Enter Quote ID: " + input_ID, skipToQuote_msg_color, true, false);
            G.info_add_str("Quote ID must be a integer from 0~" + to_string(ID_MAX),
        ...
        default:
        {
            ...
            {
                if (current_info_input<KEY_0 or current_info_input > KEY_9)
                {   // not a number
                    input_error = true;
                }
                else
...
void game_engine::remove_best_record(const int& ask_again, const string& msg)
{
    ...
    if (current_info_input == KEY_y)
    {
        ...
        {
            G.info_add_str("Are you " + really + " sure that you want to remove this record? (y/n)",
        ...
        if (isDemo)
        {   // remove best record demo
            demo_best_record = pair<double, pair<vector<long long>, vector<int>>>();
        }
        else
        {
        ...
        best_record = saver.pload(quote_order[quoteOrder_index]);
        current_game.load_game_info(current_quote, current_lc, best_record.first, G.win_w, false);
        intro_info_wait = false;
    }
}
...
void game_engine::show_bottom_msg(const string& msg, const bool& show_id, const int& nspilt_bar)
{   //msg not supposed to be longer than 2 x G.inner_info_w
    ...
    if (show_id)
    {
        bottom_msg += " | ID: " + ((isDemo) ? "DEMO" : to_string(quote_order[quoteOrder_index]));
    }

    if (bottom_msg.length() < G.inner_info_w)
    {
        G.info_mvadd_str(G.inner_info_h - 1, G.inner_info_w - (int)bottom_msg.length(),
                         bottom_msg,
                         intro_msg_color, false, true);
    }
    else
    {   // find the nth-split bar '|' and print in newline after it
        int bar_count = 0, i = 0;
        ...
        G.info_mvadd_str(G.inner_info_h - 2, G.inner_info_w - (int)msg_p1.length(),
                         msg_p1,
                         intro_msg_color, false, false);
        G.info_mvadd_str(G.inner_info_h - 1, G.inner_info_w - (int)msg_p2.length(),
                         msg_p2,
                         intro_msg_color, false, true);
    }
}
...
//{
//    // calculate
//    auto current_wpm = current_game.calculate_gross_WPM(inner_stopwatch.split(), current_game.correct_part_len);
//
//    // temporary save
//    // inGame_WPM.push_back(current_wpm);
//
//    // show
//    G.info_add_str("gross WPM:" + to_string(current_wpm), "yellow", true, true);
//}

// some removed help msg
//string general = ("HELP & TIPS\n"
//                  "On the Start Screen or End Screen, the hyphen(-) key will launch Search mode and allow "
//                  "you to skip to any quote in the current jumble of quotes by inputting its ID(number)."
//                  "\n\nOn the Start Screen only, you can press the Number key 0 to launch Challenge mode (Next page for details)"
//                  "or the DELETE key to remove the current quote\'s Best Record if it exists."
//                  "A message asking for confirmation will appear. On the End Screen only, you can press r to replay your most recent Game play. You can then choose to"
//                  "Skip it by choosing SkipToEnd in the Menu."
//                  "\n\nYou can Open Menu at almost anytime by pressing the tab key. The Shortcut to every option in Menu can be"
//                  "accessed by pressing the First Letter of each option."
//                  "\n\nNumber keys can be used to move to the n-th option. a and d are usually synonymous with ArrowNavigate Through Records by pressing Arrow Up/Down or going Up/Down page by page"
//                  "by pressing w and s. Game Statistics can be accessed by pressing 0 here."
//                  "\n\nHistory records can be Sorted by using the left and right keys to move across the column, then pressing"
//                  "Return. When sorting by different columns, if possible, the previous order will be preserved."
//                  );
//
//vector<pair<string, string>> General =
//{
//    {"HELP & TIPS\nOn the Start Screen or End Screen, the hyphen(-) key will launch ", "white"},
//    {"Search mode", "yellow"},
