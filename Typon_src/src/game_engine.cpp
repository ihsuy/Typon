...
game_engine::game_engine(const int &id_max,
                         ...
                         const int &user_defined_w, const int &user_defined_h,
                         const bool &shuffle)
    : ID_MAX(id_max), G(user_defined_w, user_defined_h),
      saver(user_defined_quote_dir), q_loader(id_max, user_defined_quote_dir) {
...
void game_engine::next_quote(const int &tar_id) {
  ...
  if (tar_id < -1) {
    Exit(1, ("game_engine::next_quote: invalid tar_id: " + to_string(tar_id) +
...
void game_engine::init_mode(const string &msg) {
    ...
    if (best_record.first <= double(0)) {
      ...
      if (best_record.second.first.size() != best_record.second.second.size()) {
        ...
        if (test_len != current_lc) {
          ...
          G.info_add_str("Challange Mode Activated", challenge_warning_color,
                         true, false);
          G.info_add_str("Best Gross WPM: " + d_to_string(best_record.first, 2),
  ...
  if (mode == "Classic") {
    ...
    if (intro_info_wait) { // don't clear previous since there's extra message
      //[TODO] very inefficient work around with a boolean, improvement needed
      clear_prev = false;
...
void game_engine::timing_begin() { inner_stopwatch.start(); }
...
/*--------------------*/
// get text input
int game_engine::get_input() { return wgetch(G.inner_text_frame); }
...
void game_engine::inGame_get_input() { // ignore all characters except ascii
  ...
  if ((current_text_input < KEY_SPACE or current_text_input > 127) and
      current_text_input != KEY_MENU and current_text_input != KEY_BACKSPACE and
      current_text_input != '\b') {
...
void game_engine::progress() { // game advances 1 cycle
  ...
  if ((current_game.typed_length == current_lc and
       ...
       current_text_input != KEY_DEL and current_text_input != KEY_BACKSPACE and
       current_text_input != '\b') or
      (current_game.typed_length == 0 and
       (current_text_input == KEY_DEL or current_text_input == KEY_BACKSPACE or
        current_text_input == '\b'))) {
    return;
  }
...
void game_engine::print_record(const int &highlight,
  ...
  if (history_rec.size() == 0) { // if no record in gsave
    ...
    if (not isDemo and verticle_move) {
      ...
      temp_quote =
          q_loader.load_quote(highlighted_quote_id, inHistory_quoteGlancee_len);

      if (temp_quote == "") {
        G.erase_info();
        G.info_mvadd_str(0, 0, "Quote not found.", inHistory_quoteGlance_color,
                         false, false);
      } else {
        G.erase_info();
        G.info_mvadd_str(0, 0, temp_quote + "...", inHistory_quoteGlance_color,
                         false, false);
      }
    }
  }
...
void game_engine::inHistory_recordChoiceMove(const int &val, long long &start_i,
                                             long long &end_i) {
  verticle_move = true;
...
void game_engine::inHistory_skipToQuote() {
  ...
  if (current_info_input == KEY_y) {
    ...
    if (temp_quote == "") {
      G.info_mvadd_str(2, 0, "Quote not found. Press anything to continue.",
                       "black-yellow");
      inInfo_get_input();
    } else {
...
void game_engine::skipToQuote() {
  ...
  while (true) {
    if (input_error) {
      G.info_add_str("Enter Quote ID: " + input_ID, skipToQuote_msg_color, true,
                     false);
    ...
    default: {
      if (current_info_input == KEY_DEL or
          current_info_input == KEY_BACKSPACE or current_info_input == '\b') {
        if (input_ID.size() != 0) {
          input_ID.pop_back();
          input_error = false;
        }
...
void game_engine::show_bottom_msg(
    const string &msg, const bool &show_id,
    const int
  ...
  if (bottom_msg.length() < G.inner_info_w) {
    ...
    G.info_mvadd_str(G.inner_info_h - 2, G.inner_info_w - (int)msg_p1.length(),
                     msg_p1, intro_msg_color, false, false);
    G.info_mvadd_str(G.inner_info_h - 1, G.inner_info_w - (int)msg_p2.length(),
                     msg_p2, intro_msg_color, false, true);
  }
}
