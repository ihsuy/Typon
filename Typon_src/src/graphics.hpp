...
struct Graphics {
  ...
  /*--------------------*/
  // write text
  //    void add_msg(const vector<string>& contents, const vector<string> color,
  //    const string& win, const bool& clear_prev=true, const bool&
  //    refresh=true);

  void text_add_str(const string &str, const string &c,
                    const bool clear_prev = false, const bool &refresh = false);

  void text_mvadd_str(const int &y, const int &x, const string &str,
                      const string &c, const bool clear_prev = false,
                      const bool &refresh = false);

  void text_add_ch(const char &ch, const string &c,
                   const bool clear_prev = false, const bool &refresh = false);

  /*--------------------*/
  // write msg
  void info_add_str(const string &str, const string &c,
                    const bool &clear_prev = false, const bool &refresh = true,
                    const string &sep = "");

  void info_mvadd_str(const int &y, const int &x, const string &str,
                      const string &c, const bool &clear_prev = false,
                      const bool &refresh = true);

  void info_add_ch(const char &ch, const string &c,
                   const bool &clear_prev = false, const bool &refresh = true);

  void info_mvadd_ch(const int &y, const int &x, const char &ch,
                     const string &c, const bool &clear_prev = false,
                     const bool &refresh = true);
  ...
  /*--------------------*/
  // replay utilites
  void draw_keyboard(const int &y, const int &x, const int &key,
                     const string &c);

  void mvclrtobot(const string &win, const int &y, const int &x,
                  const bool &refresh = false);
