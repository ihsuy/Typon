...
class game_engine
{
    ...
    const string inMenu_bottom_msg = "Close Menu: tab | LEFT/RIGHT: a/d or arrow keys | Choose: return";
    const string inHistory_bottom_msg = "Menu: tab | Navigate: 4 arrow keys | Sort: return | goto: space | Page Up/Down: w/s | Stats: 0";
    ...
    void replaySkip();
    void skipToQuote();
    void remove_best_record(const int& ask_again = 0, const string& msg = "Remove record for this quote?");
    void show_stats();
    void quit_history();
    void quit_help();
    ...
    void show_bottom_msg(const string& msg, const bool& show_id = true, const int& nspilt_bar = 3);
    ...
    void inHistory_recordChoiceMove(const int& val, long long& start_i, long long& end_i);
