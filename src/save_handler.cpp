...
bool id_ascend(const string& line1, const string& line2)
{
    ...
    {
        throw Conversion_error("Reordering History records. (please check gsave ID format)");
    }
}
...
bool wpm_ascend(const string& line1, const string& line2)
{
    ...
    {
        throw Conversion_error("Reordering History records. (please check gsave WPM format)");
    }
}
...
bool mistake_ascend(const string& line1, const string& line2)
{
    ...
    {
        throw Conversion_error("Reordering History records. (please check gsave Mistake format)");
    }
}
...
void save_handler::validate_gsave()
{
    ...
    string temp_gaddr = gsave_path.substr(0, psave_path.length()-save_fmt.length()) + temp_suffix;
...
void save_handler::validate_psave(const vector<int>& quote_order)
{   // check performance record size
    ...
    else
    {
        while(true)
        {
            ...
            {
                throw Conversion_error("Validating psave file. (please check psave ID format)");
            }
    ...
    {   // some quotes are removed,
        ...
        if(user_inp != "y")
        {
            Exit(0, "NOTE:\nIf you are modifying the path to your \"quotes\" folder,\nalso migrate the quote files from your previous directory to keep their record.");
        }
    }
    
    string temp_paddr = psave_path.substr(0, psave_path.length()-save_fmt.length()) + temp_suffix;
...
void save_handler::save(const int& id, const double& wpm, const int& mis, const vector<long long>& rec, const vector<int>& all_inputs)
{
    ...
    //                 "Mistakes: " + to_string(mis) + "  " +
    //                 "Date: " + time_string;
    //
    //    gsave_file << msg;
    
    if(gsave_len != 0) gsave_file << "\n";
...
vector<string> save_handler::gload(const long long& start_i, const long long& end_i, const string& order)
{
    ...
    if(start_i > end_i)
    {
        Exit(1, "save_handler::gload: start_i ("+to_string(start_i)+") can't be larger than end_i("+to_string(end_i)+").");
    }
    ...
    if(gfr_size < gsave_len)
    {
        ...
        gload_full_record.insert(gload_full_record.end(), data_lines.begin(), data_lines.end());
...
pair<double, pair<vector<long long>, vector<int>>> save_handler::pload(const int& id)
{
    ...
    while(getline(psave_file, line))
    {
        ...
        if(token == "Quote_id:")
        {
            ...
            if(token == to_string(id))
            {
                ...
                // next token is "rec:" and the next after that is serie of double records
                ss >> token >> token;
                while(token != "input:")
                {
                    int time_token;
                    ...
                    {
                        throw Conversion_error("Loading psave. (Please check psave rec format)");
                    }
    ...
    return pair<double, pair<vector<long long>, vector<int>>> {wpm, pair<vector<long long>, vector<int>>(rec, inp)};
}
...
void save_handler::save_stats(const long long& last_game_nchar)
{
    ...
    if(!buffer.eof())
    {
        ...
        {
            throw Conversion_error("loading First 20 games average gross wpm. (please check stats oldwpm format)");
        }
        ...
        if(!buffer.eof())
        {
            ...
            {
                throw Conversion_error("loading First 20 games average mistakes. (please check stats oldmistake format)");
            }
    ...
    if(not oldwpm_calculated or not oldmistake_calculated)
    {
        ...
        for(auto i = 0; i < old_range; ++ i)
        {
            ...
            {
                throw Conversion_error("Calculating first 20 games average gross wpm. (please check gsave WPM format)");
            }
            ...
            {
                throw Conversion_error("Calculating first 20 games average gross mistakes. (please check gsave mistakes format)");
            }
        }
    ...
    for(auto i = 0; i < gsave_len; ++i)
    {
        ...
        {
            throw Conversion_error("Calculating All time average gross wpm. (please check gsave WPM format)");
        }
        ...
        if(i >= gsave_len - recent_range)
        {
            ...
            {
                throw Conversion_error("Calculating Recent 20 games average wpm. (please check gsave WPM format)");
            }
        }
        ...
        {
            throw Conversion_error("Calculating All time average mistakes. (please check gsave mistakes format)");
        }
        ...
        if(i >= gsave_len - recent_range)
        {
            ...
            {
                throw Conversion_error("Calculating Recent 20 games average mistakes (please check gsave mistakes format)");
            }
        }
    }
    ...
    {
        throw Conversion_error("Updating All time in game key pressed. (please check stats totalchar format)");
    }
    
    perf["totalchar"] = temp_d1;
    
    string temp_saddr = stats_path.substr(0, stats_path.length()-save_fmt.length()) + temp_suffix;
    ofstream temp_stats(temp_saddr);
    for(auto i = 0; i < stats_entries.size()-1; ++i)
    {
        temp_stats << stats_entries[i] << ' ' << perf[stats_entries[i]] << '\n';
    }
    temp_stats << stats_entries.back() << ' '
    << setprecision(nchar_precision)
...
// disabled
//vector<string> save_handler::gload_windowed(const long long& start_i, const long long& end_i, const bool& include_header)
//{
//    if(start_i > end_i)
//    {
//        throw runtime_error("save_handler::gload: start_i can't be larger than end_i");
//    }
//    else if(start_i == end_i)
//    {
//        return vector<string>{};
...
//
//    //
//    //    cout << "c_start: " << cache_start << " c_end: " << cache_end << endl;
//    //    cout << "start_1: " << start_i << " end_i: " << end_i << endl;
//    //cout << "cache_start: " << cache_start << " cache_end: " << cache_end << "save_len" << gsave_len << endl;
//    if(gload_cache.size() == 0
...
//            res.insert(res.begin()+1, gload_cache.begin()+ start_i - cache_start, gload_cache.begin() + end_i - cache_start);
//        }
//        else{
//            res = vector<string>(gload_cache.begin()+(start_i - cache_start), gload_cache.begin() + end_i - cache_start);
//        }
//
//        // slice
//        return res;
