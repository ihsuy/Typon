...
struct Game_mode
{
    ...
    /*--------------------*/
    // prepare a vector object to record all input chars except those were correct but deleted and retyped
    // mark correct chars as 0 incorrect chars as 1
    vector<pair<int, int>> total_input;
    vector<pair<int, int>> get_total_input();
    
    /*--------------------------------------------------*/
    // common tools
    void load_game_info(const string& c_quote, const unsigned long& c_lc, const double& best_wpm, const int& win_w, const bool& do_prettify=true);
    ...
    double calculate_net_WPM(const double& t, const unsigned long& lc, const unsigned int& miss_count);
