...
class game_engine {
  ...
  const string inHistory_bottom_msg =
      "Menu: tab | Navigate: 4 arrow keys | Sort: return | goto: space | Page "
      "Up/Down: w/s | Stats: 0";
  ...
  const string demo_quote =
      ...
      "quotes or correct the path to your quotes by using the \"-path\" flag.";
  ...
  void replaySkip();
  void skipToQuote();
  void remove_best_record(const int &ask_again = 0,
  ...
  void show_bottom_msg(const string &msg, const bool &show_id = true,
  ...
  void inHistory_recordChoiceMove(const int &val, long long &start_i,
                                  long long &end_i);
