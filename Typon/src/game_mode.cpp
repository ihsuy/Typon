...
void Game_mode::load_game_info(const string &c_quote, const unsigned long &c_lc,
                               const double &best_wpm, const int &win_w,
...
vector<pair<int, int>> Game_mode::get_total_input() { return total_input; }
...
double Game_mode::calculate_net_WPM(const double &t, const unsigned long &lc,
    ...
    case 0:
      parts = {
          ...
          make_pair(current_quote_for_display.substr(correct_part_len, 1),
                    cursor_color), // cursor
          make_pair(
              current_quote_for_display.substr((correct_part_len == current_lc)
                                                   ? correct_part_len
                                                   : correct_part_len + 1),
              untyped_color) // untyped part
      };
    ...
    case 2:
      parts = {
          make_pair(current_quote_for_display.substr(0, correct_part_len),
                    correct_color), // correct part
          make_pair(current_quote_for_display.substr(
                        correct_part_len, opponents_len - correct_part_len - 1),
...
void Game_mode::display_performance(Graphics &g) {
  ...
  if (gross_wpm > best_WPM) {
    ...
    g.info_add_str("History best WPM: " + d_to_string(best_WPM, 2), "yellow",
                   false, false, "\t");
  }
...
bool Game_mode::game_complete() { return (correct_part_len == current_lc); }
