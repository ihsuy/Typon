...
void Graphics::init_size_params(const int &user_defined_w,
  ...
  if (actual_term_w < term_w_absolutemin or
    ...
    if (user_defined_w < term_w_absolutemin or
      ...
      Exit(1, "Please resize Terminal window to\n" + to_string(user_defined_w) +
                  "x" + to_string(user_defined_h) + " or larger.");
    } else {
...
Graphics::Graphics(const int &w, const int &h) { init_size_params(w, h); }
...
void Graphics::text_add_str(const string &str, const string &c,
                            const bool clear_prev, const bool &refresh) {
...
void Graphics::text_mvadd_str(const int &y, const int &x, const string &str,
                              const string &c, const bool clear_prev,
                              const bool &refresh) {
...
void Graphics::text_add_ch(const char &ch, const string &c,
                           const bool clear_prev, const bool &refresh) {
...
void Graphics::info_add_str(const string &str, const string &c,
                            const bool &clear_prev, const bool &refresh,
                            const string &sep) {
  if (clear_prev)
    erase_info();
...
void Graphics::info_mvadd_str(const int &y, const int &x, const string &str,
                              const string &c, const bool &clear_prev,
                              const bool &refresh) {
  if (clear_prev)
    erase_info();
...
void Graphics::info_add_ch(const char &ch, const string &c,
                           const bool &clear_prev, const bool &refresh) {
  if (clear_prev)
    erase_info();
...
void Graphics::info_mvadd_ch(const int &y, const int &x, const char &ch,
                             const string &c, const bool &clear_prev,
                             const bool &refresh) {
  if (clear_prev)
    erase_info();
...
void Graphics::draw_keyboard(const int &y, const int &x, const int &key,
                             const string &c) {
...
void Graphics::mvclrtobot(const string &win, const int &y, const int &x,
                          const bool &refresh) {
  int temp_y, temp_x;
...
void Graphics::verify_terminal_size() {
  ...
  if ((actual_term_w < min_terminal_w) or (actual_term_h < min_terminal_h)) {
    Exit(1, "Current Terminal size COL x ROW (Width x Height): " +
                to_string(size.ws_col) + "x" + to_string(size.ws_row) +
                "\nPlease resize Terminal window to " +
                to_string(min_terminal_w) + "x" + to_string(min_terminal_h) +
                " or larger.");
  }
}
