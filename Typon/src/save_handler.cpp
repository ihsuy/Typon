...
void save_handler::save(const int &id, const double &wpm, const int &mis,
...
save_handler::pload(const int &id) {
  ...
  while (getline(psave_file, line)) {
    ...
    if (token == "Quote_id:") {
      ...
      if (token == to_string(id)) {
        ...
        // next token is "rec:" and the next after that is serie of double
        // records
        ss >> token >> token;
        while (token != "input:") {
          int time_token;
...
void save_handler::save_stats(const long long &last_game_nchar) {
  ...
  if (!buffer.eof()) {
    ...
    try {
      ...
      throw Conversion_error("loading First 20 games average gross wpm. "
    ...
    if (!buffer.eof()) {
      ...
      try {
        ...
        throw Conversion_error("loading First 20 games average mistakes. "
  ...
  if (not oldwpm_calculated or not oldmistake_calculated) {
    ...
    for (auto i = 0; i < old_range; ++i) {
      ...
      try {
        ...
        throw Conversion_error("Calculating first 20 games average gross wpm. "
      ...
      try {
        ...
        throw Conversion_error(
            "Calculating first 20 games average gross mistakes. (please check "
  ...
  for (auto i = 0; i < gsave_len; ++i) {
    ...
    try {
      ...
      throw Conversion_error("Calculating All time average gross wpm. (please "
    ...
    if (i >= gsave_len - recent_range) {
      try {
        ...
        throw Conversion_error("Calculating Recent 20 games average wpm. "
                               "(please check gsave WPM format)");
      }
    }
    ...
    try {
      ...
      throw Conversion_error("Calculating All time average mistakes. (please "
    ...
    if (i >= gsave_len - recent_range) {
      try {
        ...
        throw Conversion_error("Calculating Recent 20 games average mistakes "
  ...
  try {
    ...
    throw Conversion_error("Updating All time in game key pressed. (please "
