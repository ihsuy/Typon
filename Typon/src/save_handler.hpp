...
struct save_handler {
  ...
  void save(const int &id, const double &wpm, const int &mis,
            const vector<long long> &rec, const vector<int> &all_inputs);

  vector<string> gload(const long long &start_i, const long long &end_i,
